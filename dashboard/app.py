...
from observability.api import baselines, repository
from observability.demo import SERVICES, seed_demo_data
...
def render_overview() -> None:
    ...
    fourth.metric("Open alerts", len([a for a in alerts if a.status == "open"]))
...
def render_evaluations() -> None:
    ...
    rows = []
    for result in evals:
        baseline = baselines.repository.get_baseline(result.service) or {}
        for metric, current in result.scores.items():
            rows.append({"Service": result.service, "Metric": metric, "Current": current, "Baseline": baseline.get(metric), "Evaluator": result.evaluator, "Samples": result.sample_count})
    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True)
    if alerts:
        st.subheader("Regression alerts")
        st.dataframe(pd.DataFrame([alert.model_dump() for alert in alerts]), use_container_width=True, hide_index=True)
...
def render_cost() -> None:
    ...
    left.metric("Projected monthly spend", f"${float(frame['estimated_cost_usd'].sum()) * 30:.2f}")
...
def render_security() -> None:
    ...
    st.metric("Observed security traces", len([t for t in traces if t.service == "llm-security-proxy"]))
